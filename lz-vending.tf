...
module "lz_vending" {
  ...
  user_managed_identities = merge(
    ...
    var.provision_kubelet_identity ? {
      aks-kubelet = {
    ...
    } : {},
    {
      ...
      workload-eso = {
        ...
        location           = var.region
        tags               = local.common_tags
        role_assignments = var.enable_eso ? {
          kv_secrets_user = {
            definition     = "Key Vault Secrets User"
            relative_scope = "/resourceGroups/${data.azurerm_key_vault.eso[0].resource_group_name}/providers/Microsoft.KeyVault/vaults/${data.azurerm_key_vault.eso[0].name}"
          }
        } : {}
      }
    }
